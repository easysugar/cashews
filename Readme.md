...
cache.setup("redis://0.0.0.0", client_side=True, client_side_prefix="my_prefix:")
```
...
** Warning ** `cache.keys_match` and `cache.get_match` don't work with this storage (works only if shards are disabled)
...
async for key in cache.keys_match("pattern:*"):
    ...
async for key, value in cache.get_match("pattern:*", count=1000):
    ...
